...
Met het script ``dump_data.sh`` kan de data van alle componenten (api) worden geëxporteerd naar een sql of csv bestand(en).

Dit script is niet bedoeld voor een data migratie naar een andere Referentielijsten Api instantie.
...
.. code-block:: shell
    ...
    DB_HOST=localhost DB_NAME=referentielijsten ./bin/dump_data.sh
